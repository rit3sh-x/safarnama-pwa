...
export const useRateBlog = () => {
  ...
  const rateBlog = useMutation(
    api.methods.ratings.rate
  ).withOptimisticUpdate((localStore, args) => {
    const current = localStore.getQuery(api.methods.ratings.getForBlog, {
      blogId: args.blogId,
    })
    if (!current) return
    ...
    const newSum = oldUserRating !== null
      ? oldSum - oldUserRating + args.rating
      : oldSum + args.rating

    localStore.setQuery(api.methods.ratings.getForBlog, { blogId: args.blogId }, {
  ...
  const mutate = async (
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : "Failed to rate blog")
    } finally {
      setIsPending(false)
    }
  }
