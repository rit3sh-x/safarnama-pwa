  ...
  const computeSplits = useCallback(() => {
    ...
    switch (splitType) {
      ...
      case "equal": {
        ...
        const share = Math.round((totalAmount / selected.length) * 100) / 100
        const remainder =
          Math.round((totalAmount - share * selected.length) * 100) / 100
      ...
      case "exact": {
        const splits = selected.map((m) => ({
          userId: m.userId,
          owedAmount: Number.parseFloat(exactAmounts[m.userId] ?? "0") || 0,
        }))
      ...
      case "percentage": {
        const splits = selected.map((m) => {
          const pct = Number.parseFloat(percentages[m.userId] ?? "0") || 0
          return {
            userId: m.userId,
  ...
  const validate = () => {
    ...
    if (splitType === "exact") {
      const sum = Array.from(selectedMembers).reduce(
        (s, id) => s + (Number.parseFloat(exactAmounts[id] ?? "0") || 0),
        0
      )
        ...
        <CredenzaBody className="space-y-4 overflow-y-auto">
              ...
              <div className="flex flex-wrap gap-1.5">
                {SPLIT_OPTIONS.map((opt) => (
                  <Button
                    key={opt.value}
                    size="sm"
                    variant={splitType === opt.value ? "default" : "outline"}
          ...
          {splitType !== "payer_only" && (
            <div className="space-y-2">
                ...
                {members.map((m) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {checked && splitType === "equal" && totalAmount > 0 && (
                        <span className="text-xs text-muted-foreground">
                          ₹{(totalAmount / selected.length).toFixed(2)}
                        </span>
                      )}
                    </div>
                  )
              ...
              {errors.split && <FieldError>{errors.split}</FieldError>}
            </div>
          )}
        </CredenzaBody>
